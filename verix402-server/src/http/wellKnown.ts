...
function setVersionHeader(res: Response): void {
  if ('set' in res && typeof res.set === 'function') res.set('x-verix402-version', '0.9.3');
  else if ('setHeader' in res && typeof res.setHeader === 'function') res.setHeader('x-verix402-version', '0.9.3');
}
...
export function handleWellKnown(_req: Request, res: Response): void {
  ...
  res.status(200).json({
    ...
    rights: {
      ...
      claim_schema: 'urn:verix402:claims:0.1',
      registry: null,
      notes:
